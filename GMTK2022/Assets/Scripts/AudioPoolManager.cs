...
public class AudioPoolManager : PoolManager
{
    ...
    public AudioSourceController ActivateAudioSource(SoundEffect sound, float pitchMultiplier = 1f, float volumeMultiplier = 1f, bool loop = false, float loopDuration = 0.0f) {
        for (int i = 0; i < transform.childCount; i++) {
            Transform child = transform.GetChild(i);
            if (!child.gameObject.activeSelf) {
                AudioSourceController controller = child.GetComponent<AudioSourceController>();
                controller.SetSound(sound, pitchMultiplier, volumeMultiplier);
